        ...
        `🔍 *Осмотр и консультация*\n\nСтоимость: 500 ₽\n\nВыберите врача:`,
        {
          attachments: [
            Keyboard.inlineKeyboard([
              [
                Keyboard.button.callback(
                  "👨‍⚕️ Dr. Иванов (терапевт)",
                  "demo_dentist_doc_ivanov",
                ),
              ],
              [
                Keyboard.button.callback(
                  "👩‍⚕️ Dr. Петрова (терапевт)",
...
// ========== ГЛАВНЫЙ ОБРАБОТЧИК ДЕМО-ДЕЙСТВИЙ ==========
async function handleDemoAction(ctx, userId, data, userStates) {
  ...
  if (data === "demo_delivery")
    return startDemoDelivery(ctx, userId, userStates);
  ...
  if (data.startsWith("demo_hair_"))
    return handleDemoHairStep(ctx, userId, data, userStates);
  if (data.startsWith("demo_delivery_"))
    return handleDemoDeliveryStep(ctx, userId, data, userStates);
  if (data.startsWith("demo_dentist_"))
    return handleDemoDentistStep(ctx, userId, data, userStates);
}

// ========== ОБРАБОТКА ТЕКСТОВОГО ВВОДА В ДЕМО ==========
async function handleDemoTextInput(ctx, userId, text, userStates) {
  const state = userStates.get(userId);
  if (
    !state ||
    state.mode !== "demo" ||
    state.demo_type !== "delivery" ||
    state.step !== "address"
  ) {
    return false;
  }
