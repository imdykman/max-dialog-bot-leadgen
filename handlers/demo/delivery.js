...
async function handleDemoDeliveryStep(ctx, userId, action, userStates) {
  ...
  switch (action) {
          ...
          Keyboard.inlineKeyboard([
            [
              Keyboard.button.callback(
                "🔸 Маленькая (25 см) — 490₽",
                "demo_delivery_pizza_small",
              ),
            ],
            [
              Keyboard.button.callback(
                "🔸 Средняя (30 см) — 690₽",
                "demo_delivery_pizza_medium",
              ),
            ],
            [
              Keyboard.button.callback(
                "🔸 Большая (35 см) — 890₽",
                "demo_delivery_pizza_large",
              ),
            ],
    ...
    case "demo_delivery_pizza_large": {
      ...
      state.data.items.push(`Пицца ${sizes[action].name}`);
      state.data.total += sizes[action].price;
    ...
    case "demo_delivery_burgers":
      await ctx.reply(`🍔 *Бургеры*`, {
        attachments: [
          Keyboard.inlineKeyboard([
            [
              Keyboard.button.callback(
                "🍔 Классический — 350₽",
                "demo_delivery_burger_classic",
              ),
            ],
            [
              Keyboard.button.callback(
                "🍔 Чизбургер — 390₽",
                "demo_delivery_burger_cheese",
              ),
            ],
            [
              Keyboard.button.callback(
                "🍔 Двойной — 490₽",
                "demo_delivery_burger_double",
              ),
            ],
            [Keyboard.button.callback("⬅️ Назад", "demo_delivery")],
          ]),
    ...
    case "demo_delivery_burger_double": {
      ...
      state.data.items.push(`Бургер ${burgers[action].name}`);
      state.data.total += burgers[action].price;
      await showDeliveryCart(ctx, state);
      break;
    }

    // ========== САЛАТЫ (НОВОЕ!) ==========
    case "demo_delivery_salads":
      await ctx.reply(`🥗 *Салаты*`, {
        attachments: [
          Keyboard.inlineKeyboard([
            [
              Keyboard.button.callback(
                "🥗 Цезарь — 320₽",
                "demo_delivery_salad_caesar",
              ),
            ],
            [
              Keyboard.button.callback(
                "🥗 Греческий — 290₽",
                "demo_delivery_salad_greek",
              ),
            ],
            [
              Keyboard.button.callback(
                "🥗 Оливье — 250₽",
                "demo_delivery_salad_olivier",
              ),
            ],
            [Keyboard.button.callback("⬅️ Назад", "demo_delivery")],
          ]),
      ...
      break;
    case "demo_delivery_salad_caesar":
    case "demo_delivery_salad_greek":
    case "demo_delivery_salad_olivier": {
      const salads = {
        demo_delivery_salad_caesar: { name: "Цезарь", price: 320 },
        demo_delivery_salad_greek: { name: "Греческий", price: 290 },
        demo_delivery_salad_olivier: { name: "Оливье", price: 250 },
      };
      state.data.items.push(`Салат ${salads[action].name}`);
      state.data.total += salads[action].price;
      await showDeliveryCart(ctx, state);
      break;
    }

    // ========== НАПИТКИ (НОВОЕ!) ==========
    case "demo_delivery_drinks":
        ...
        attachments: [
          Keyboard.inlineKeyboard([
            [
              Keyboard.button.callback(
                "🥤 Кола — 120₽",
                "demo_delivery_drink_cola",
              ),
            ],
            [
              Keyboard.button.callback(
                "🧃 Сок — 150₽",
                "demo_delivery_drink_juice",
              ),
            ],
            [
              Keyboard.button.callback(
                "💧 Вода — 80₽",
                "demo_delivery_drink_water",
              ),
            ],
            [Keyboard.button.callback("⬅️ Назад", "demo_delivery")],
          ]),
      ...
      break;
    case "demo_delivery_drink_cola":
    case "demo_delivery_drink_juice":
    case "demo_delivery_drink_water": {
      const drinks = {
        demo_delivery_drink_cola: { name: "Кола", price: 120 },
        demo_delivery_drink_juice: { name: "Сок", price: 150 },
        demo_delivery_drink_water: { name: "Вода", price: 80 },
      };
      state.data.items.push(`Напиток: ${drinks[action].name}`);
      state.data.total += drinks[action].price;
      await showDeliveryCart(ctx, state);
      break;
    }
