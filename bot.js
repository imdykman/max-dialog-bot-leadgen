...
function isValidPhone(input) {
  ...
  // Вторая цифра должна быть 9 (код мобильного оператора в РФ)
  if (normalized[1] !== "9") {
    return false;
  }
...
async function handleOrderStep(ctx, userId, data) {
  ...
  switch (state.step) {
    ...
    case ORDER_STEPS.CONTACT:
      ...
      if (/^\d+$/.test(data.trim())) {
        // Валидируем формат телефона (должно быть 10 или 11 цифр)
...
bot.on("message_callback", async (ctx) => {
  ...
  if (data === "change_contact") {
    const state = userStates.get(userId);
    if (state && state.mode === "order") {
      state.step = ORDER_STEPS.CONTACT;
      userStates.set(userId, state);
      await ctx.reply(`📱 Введите новый телефон или @username:`);
    }
    return;
  }
